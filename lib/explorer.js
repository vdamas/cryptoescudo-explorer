...
module.exports = {
  ...
  get_supply: function(cb) {
    if (settings.use_rpc) {
      ...
      } else if (settings.supply == 'BLOCKNUM') {
        var uri = base_url + 'getblockcount';
        request({uri: uri, json: true}, function (error, response, body) {
          var coinsupply = 0;
          var blocknum = body;
          for(i = 1; i < 11; i++) { coinsupply += 22000000; }
          for(i = 11; i < 21; i++) { coinsupply += 22500000; }
          for(i = 21; i < 31; i++) { coinsupply += 500000; }
          for(i = 31; i < 46; i++) { coinsupply += 20; }
          for(i = 46; i < 164000; i++) { coinsupply += 600; }
          for(i = 164000; i < 1314046; i++) { if( i <= blocknum ) { coinsupply += 200; } else { break;}}
          for(i = 1314046; i < blocknum; i++) { coinsupply += 20; }
          return cb(coinsupply);
        });
      } else {
        coinbase_supply(function(supply) {
          return cb(supply/100000000);
